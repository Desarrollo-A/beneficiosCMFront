...
import { getDatosEvento,updateAsistenciaEvento } from 'src/api/asistenciaEv/asistenciaEv'; 
...
export default function EventScannerDialog({ open, onClose, mutate }) {
  ...
  return (
    <>
      ...
      >
        <DialogTitle sx={{ pb: 2 }}>Escanear código QR de asistencia</DialogTitle>
      ...
      <ConfirmDialog
        ...
        content={
          <Typography>
            {data && data?.result
              ? `¿Deseas confirmar asistencia de ${data?.data[0]?.nombreCompleto} para ${data?.data[0]?.titulo}?`
              : data?.msg}
          </Typography>
        }
        action={
          <>
            ...
            <LoadingButton
              variant="contained"
              color="success"
              onClick={() => handleConfirmAssistance(data?.data[0]?.idEvento, data?.data[0]?.idcontrato)} 
              loading={false}
            >
              Aceptar
            </LoadingButton>
